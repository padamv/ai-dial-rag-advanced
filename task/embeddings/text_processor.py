...
class TextProcessor:
    ...
    def search(
        ...
        embeddings = self.embeddings_client.get_embeddings(inputs=user_request, dimensions=dimensions)
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                query = self._get_search_query(search_mode)
                cursor.execute(query=query, vars=(vector_string, vector_string, max_distance, top_k))
